...
from utils import MinMaxStats, Action, ActionHistory, Node, SharedStorage, ReplayBuffer, softmax_sample

# Each self-play job is independent of all others; it takess the latest network
# snapshot, produces a game and makes it available to the training job by writing it to a shared replay buffer.
...
# repeatedly executing a Monte Carlo Tree Search to generate moves until the end of the game is reached.
def play_game(config: MuZeroConfig, network: Network) -> Game:
    game = config.new_game()

    while not game.terminal() and len(game.history) < config.max_moves:
        # At the root of the search tree we use the representation model to
...
def run_mcts(config: MuZeroConfig, root: Node, action_history: ActionHistory, network: Network):
    ...
    for _ in range(config.num_simulations):
        ...
        # Inside the search tree we use the dynamics model to obtain the next
        # hidden state given an action and the previous hidden state.
        parent = search_path[-2]
        network_output = network.reccurent_inference(parent.hidden_state, history.last_action())
